...
static int bkpfs_restore(struct file *file, const int version_num)
{
	int err = 0, vfs_err = 0;
	...
	int lower_namelen, loff_t offset, u64 ino, unsigned int d_type)
{
	...
	rc = strncmp(lower_name, "backup.", 7);
 ...
 */
static int
bkpfs_view(struct file *file, const int operation_flag,
...
static int bkpfs_open(struct inode *inode, struct file *file)
{
	...
	rc = strncmp(file->f_path.dentry->d_name.name, "backup.", 7);
	if (!rc) {
